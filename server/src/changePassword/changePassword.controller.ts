...
export class PasswordController {
  ...
  async changePassword(
    ...
    if(!token){
      res.status(HttpStatus.UNAUTHORIZED).json({message: "인증되지 않았습니다."});
      return;
    }
    try {
      const decoded = this.passwordService.verifyToken(token);
      const userEmail = decoded.email;
      await this.passwordService.changePassword(userEmail, oldPassword, newPassword);
      res.status(HttpStatus.OK).json({message: "비밀번호가 성공적으로 변경되었습니다."});
    } catch (error) {
